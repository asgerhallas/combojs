  ...
  (function($, window) {
    ...
    Combo = (function() {
      ...
      function Combo(wrapper, options) {
        ...
        this.input = $(("<input type='text' class='combo-input' autocomplete='off' disabled='disabled' spellcheck='" + this.spellcheck + "' placeholder='" + this.placeholder + "' " + (this.tabIndex != null ? "tabindex='" + this.tabIndex + "'" : "") + " />") + this.getLabel()).bind({
      ...
      Combo.prototype.itemDisplay = function(item, bare) {
        var title;
        if (bare == null) {
          bare = false;
        }
        title = item.__isRawValueItem ? item.__rawValue : evaluate(this.displayField, item);
        if (bare) {
          return title;
        } else {
      ...
      Combo.prototype.itemTitle = function(item) {
        var ref, title;
        title = item.__isRawValueItem ? item.__rawValue : this.stripMarkup((ref = evaluate(this.titleField, item)) != null ? ref : this.itemDisplay(item, true));
        return title + this.getLabel();
      };
      ...
      Combo.prototype.selectItem = function(item, options) {
        ...
        this.input.val(this.stripLabel(this.itemTitle(item)));
        this.updateClassNames();
        this.lastQuery = this.input.val();
        this.updateLastSelection();
        this.internalCollapse();
      ...
      Combo.prototype.renderList = function(items, secondaryItems, filters) {
        ...
        if (this.showUnmatchedRawValue) {
          ...
          if (rawValue !== "" && !this.hasSelection()) {
            htmls.push("<li class='unmatched-raw-value'>" + (rawValue + this.getLabel()) + "</li>");
          }
        }
      ...
      Combo.prototype.getLabel = function() {
        if (this.label !== "") {
      ...
      Combo.prototype.stripLabel = function(text) {
        if (text == null) {
          text = "";
        }
        return text.replace(this.getLabel(), "");
      };
